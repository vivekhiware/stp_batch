...
@Service
public class STP_CARDS_NFS_ISS_RECON_TTUMsheduledTask {

	private static final Logger logger = LoggerFactory.getLogger(STP_CARDS_NFS_ISS_RECON_TTUMsheduledTask.class);
	@Autowired
	private static ServiceCards serviceCards;

	// Executes every minute
//	@Scheduled(cron = "0 * * * * ?")
	public void executeTask() {
		logger.info("STP_CARDS_NFS_ISS_RECON_TTUMsheduledTask Scheduled task  start executed at: " + new Date());
	}

	public static void PROCESSSTP_CARDS_NFS_ISS_RECON_TTUM() {
		...
		try {
			...
			dbresponse += processRequestSTP_CARDS_NFS_ISS_RECON_TTUM("FRESH", "L4", serviceNetwork, dbresponse); // New
																													// requests
			System.out.println("NEW REQUEST END");

			dbresponse += processRequestSTP_CARDS_NFS_ISS_RECON_TTUM("ENQUIRY", "L5", serviceNetwork, dbresponse); // Enquiry
			// requests
			System.out.println("ENQUIRED REQUEST END");

			dbresponse += processRequestSTP_CARDS_NFS_ISS_RECON_TTUM("REPEAT", "L6", serviceNetwork, dbresponse); // Repeat
			// requests
	...
	private static int processRequestSTP_CARDS_NFS_ISS_RECON_TTUM(String requestType, String condition,
			ISOCommunicationExample serviceNetwork, int dbresponse) {
		List<STP_CARDS_NFS_ISS_RECON_TTUM> requestList = null;
		...
		try {
			...
			for (Iterator<IsoV93Message> iterator = request1200.iterator(); iterator.hasNext();) {
				...
				dbresponse += serviceCards.updateSTP_CARDS_NFS_ISS_RECON_TTUM(isoV93MessageRes, requestType);
