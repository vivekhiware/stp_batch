...
@Service
public class STP_UPI_ADJUSTMENT_REPORTscheduledTask {

	private static final Logger logger = LoggerFactory.getLogger(STP_UPI_ADJUSTMENT_REPORTscheduledTask.class);
	@Autowired
...
//	@Scheduled(cron = "0 * * * * ?")
	public void executeTask() {
		logger.info("STP_UPI_ADJUSTMENT_REPORTsheduledTask Scheduled task  start executed at: " + new Date());
	}

	public static void PROCESSSTP_UPI_ADJUSTMENT_REPORT() {
		...
		try {
			...
			dbresponse += processRequestSTP_UPI_ADJUSTMENT_REPORT("FRESH", "L4", serviceNetwork, dbresponse); // New
																												// requests
			System.out.println("NEW REQUEST END");

			dbresponse += processRequestSTP_UPI_ADJUSTMENT_REPORT("ENQUIRY", "L5", serviceNetwork, dbresponse); // Enquiry
			// requests
			System.out.println("ENQUIRED REQUEST END");

			dbresponse += processRequestSTP_UPI_ADJUSTMENT_REPORT("REPEAT", "L6", serviceNetwork, dbresponse); // Repeat
			// requests
	...
	private static int processRequestSTP_UPI_ADJUSTMENT_REPORT(String requestType, String condition,
			ISOCommunicationExample serviceNetwork, int dbresponse) {
		List<STP_UPI_ADJUSTMENT_REPORT> requestList = null;
		...
		try {
			...
			for (Iterator<IsoV93Message> iterator = request1200.iterator(); iterator.hasNext();) {
				...
				dbresponse += serviceUpi.updateSTP_UPI_ADJUSTMENT_REPORT(isoV93MessageRes, requestType);
