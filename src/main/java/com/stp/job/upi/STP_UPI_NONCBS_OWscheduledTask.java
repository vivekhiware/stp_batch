...
@Service
public class STP_UPI_NONCBS_OWscheduledTask {

	private static final Logger logger = LoggerFactory.getLogger(STP_UPI_NONCBS_OWscheduledTask.class);
	@Autowired
...
//	@Scheduled(cron = "0 * * * * ?")
	public void executeTask() {
		logger.info("STP_UPI_NONCBS_OWscheduledTask Scheduled task  start executed at: " + new Date());
	}

	public static void PROCESSSTP_UPI_NONCBS_OW() {
		...
		try {
			...
			dbresponse += processRequestSTP_UPI_NONCBS_OW("FRESH", "L4", serviceNetwork, dbresponse); // New
																										// requests
			System.out.println("NEW REQUEST END");

			dbresponse += processRequestSTP_UPI_NONCBS_OW("ENQUIRY", "L5", serviceNetwork, dbresponse); // Enquiry
			// requests
			System.out.println("ENQUIRED REQUEST END");

			dbresponse += processRequestSTP_UPI_NONCBS_OW("REPEAT", "L6", serviceNetwork, dbresponse); // Repeat
			// requests
	...
	private static int processRequestSTP_UPI_NONCBS_OW(String requestType, String condition,
			ISOCommunicationExample serviceNetwork, int dbresponse) {
		List<STP_UPI_NONCBS_OW> requestList = null;
		...
		try {
			...
			for (Iterator<IsoV93Message> iterator = request1200.iterator(); iterator.hasNext();) {
				...
				dbresponse += serviceUpi.updateSTP_UPI_NONCBS_OW(isoV93MessageRes, requestType);
