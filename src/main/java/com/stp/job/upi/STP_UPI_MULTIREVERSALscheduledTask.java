...
@Service
public class STP_UPI_MULTIREVERSALscheduledTask {

	private static final Logger logger = LoggerFactory.getLogger(STP_UPI_MULTIREVERSALscheduledTask.class);
	@Autowired
...
//	@Scheduled(cron = "0 * * * * ?")
	public void executeTask() {
		logger.info("STP_UPI_MULTIREVERSALscheduledTask Scheduled task  start executed at: " + new Date());
	}

	public static void PROCESSSTP_UPI_MULTIREVERSAL() {
		...
		try {
			...
			dbresponse += processRequestSTP_UPI_MULTIREVERSAL("FRESH", "L4", serviceNetwork, dbresponse); // New
																											// requests
			System.out.println("NEW REQUEST END");

			dbresponse += processRequestSTP_UPI_MULTIREVERSAL("ENQUIRY", "L5", serviceNetwork, dbresponse); // Enquiry
			// requests
			System.out.println("ENQUIRED REQUEST END");

			dbresponse += processRequestSTP_UPI_MULTIREVERSAL("REPEAT", "L6", serviceNetwork, dbresponse); // Repeat
			// requests
	...
	private static int processRequestSTP_UPI_MULTIREVERSAL(String requestType, String condition,
			ISOCommunicationExample serviceNetwork, int dbresponse) {
		List<STP_UPI_MULTIREVERSAL> requestList = null;
		...
		try {
			...
			for (Iterator<IsoV93Message> iterator = request1200.iterator(); iterator.hasNext();) {
				...
				dbresponse += serviceUpi.updateSTP_UPI_MULTIREVERSAL(isoV93MessageRes, requestType);
