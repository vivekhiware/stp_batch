...
@Service
public class STP_IMPS_CHARGEBACK_REPORTsheduledTask {

	private static final Logger logger = LoggerFactory.getLogger(STP_IMPS_CHARGEBACK_REPORTsheduledTask.class);
	@Autowired
...
//	@Scheduled(cron = "0 * * * * ?")
	public void executeTask() {
		logger.info("STP_IMPS_CHARGEBACK_REPORTsheduledTask Scheduled task  start executed at: " + new Date());
	}

	public static void PROCESSSTP_IMPS_CHARGEBACK_REPORT() {
		...
		try {
			...
			dbresponse += processRequestSTP_IMPS_CHARGEBACK_REPORT("FRESH", "L4", serviceNetwork, dbresponse); // New
																												// requests
			System.out.println("NEW REQUEST END");

			dbresponse += processRequestSTP_IMPS_CHARGEBACK_REPORT("ENQUIRY", "L5", serviceNetwork, dbresponse); // Enquiry
			// requests
			System.out.println("ENQUIRED REQUEST END");

			dbresponse += processRequestSTP_IMPS_CHARGEBACK_REPORT("REPEAT", "L6", serviceNetwork, dbresponse); // Repeat
			// requests
	...
	private static int processRequestSTP_IMPS_CHARGEBACK_REPORT(String requestType, String condition,
			ISOCommunicationExample serviceNetwork, int dbresponse) {
		List<STP_IMPS_CHARGEBACK_REPORT> requestList = null;
		...
		try {
			...
			if ("FRESH".equals(requestType)) {
				requestList = serviceImps.processSTP_IMPS_CHARGEBACK_REPORT("L4");
				request1200 = IsoPackagerGlobal.request1200(requestList);
			} else if ("ENQUIRY".equals(requestType)) {
				requestList = serviceImps.processSTP_IMPS_CHARGEBACK_REPORT("L5");
				request1200 = IsoPackagerGlobal.request1200Enquiry(requestList);
			} else if ("REPEAT".equals(requestType)) {
				requestList = serviceImps.processSTP_IMPS_CHARGEBACK_REPORT("L6");
			...
			for (Iterator<IsoV93Message> iterator = request1200.iterator(); iterator.hasNext();) {
				...
				dbresponse += serviceImps.updateSTP_IMPS_CHARGEBACK_REPORT(isoV93MessageRes, requestType);
