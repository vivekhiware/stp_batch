...
@Service
public class STP_IMPS_RCC_REPORTsheduledTask {

	private static final Logger logger = LoggerFactory.getLogger(STP_IMPS_RCC_REPORTsheduledTask.class);
	@Autowired
...
//	@Scheduled(cron = "0 * * * * ?")
	public void executeTask() {
		logger.info("STP_IMPS_RCC_REPORTsheduledTask Scheduled task  start executed at: " + new Date());
	}

	public static void PROCESSSTP_IMPS_RCC_REPORT() {
		...
		try {
			...
			dbresponse += processRequestSTP_IMPS_RCC_REPORT("FRESH", "L4", serviceNetwork, dbresponse); // New
																										// requests
			System.out.println("NEW REQUEST END");

			dbresponse += processRequestSTP_IMPS_RCC_REPORT("ENQUIRY", "L5", serviceNetwork, dbresponse); // Enquiry
			// requests
			System.out.println("ENQUIRED REQUEST END");

			dbresponse += processRequestSTP_IMPS_RCC_REPORT("REPEAT", "L6", serviceNetwork, dbresponse); // Repeat
			// requests
	...
	private static int processRequestSTP_IMPS_RCC_REPORT(String requestType, String condition,
			ISOCommunicationExample serviceNetwork, int dbresponse) {
		List<STP_IMPS_RCC_REPORT> requestList = null;
		...
		try {
			...
			for (Iterator<IsoV93Message> iterator = request1200.iterator(); iterator.hasNext();) {
				...
				dbresponse += serviceImps.updateSTP_IMPS_RCC_REPORT(isoV93MessageRes, requestType);
