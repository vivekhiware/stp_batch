...
@Service
public class STP_IMPS_OW_NETWORK_DECLINEscheduledTask {

	private static final Logger logger = LoggerFactory.getLogger(STP_IMPS_OW_NETWORK_DECLINEscheduledTask.class);
	@Autowired
...
//	@Scheduled(cron = "0 * * * * ?")
	public void executeTask() {
		logger.info("STP_IMPS_OW_NETWORK_DECLINEscheduledTask Scheduled task  start executed at: " + new Date());
	}

	public static void PROCESSSTP_IMPS_OW_NETWORK_DECLINE() {
		...
		try {
			...
			dbresponse += processRequestSTP_IMPS_OW_NETWORK_DECLINE("FRESH", "L4", serviceNetwork, dbresponse); // New
																												// requests
			System.out.println("NEW REQUEST END");

			dbresponse += processRequestSTP_IMPS_OW_NETWORK_DECLINE("ENQUIRY", "L5", serviceNetwork, dbresponse); // Enquiry
			// requests
			System.out.println("ENQUIRED REQUEST END");

			dbresponse += processRequestSTP_IMPS_OW_NETWORK_DECLINE("REPEAT", "L6", serviceNetwork, dbresponse); // Repeat
			// requests
	...
	private static int processRequestSTP_IMPS_OW_NETWORK_DECLINE(String requestType, String condition,
			ISOCommunicationExample serviceNetwork, int dbresponse) {
		List<STP_IMPS_OW_NETWORK_DECLINE> requestList = null;
		...
		try {
			...
			if ("FRESH".equals(requestType)) {
				requestList = serviceImps.processSTP_IMPS_OW_NETWORK_DECLINE("L4");
				request1200 = IsoPackagerGlobal.request1200(requestList);
			} else if ("ENQUIRY".equals(requestType)) {
				requestList = serviceImps.processSTP_IMPS_OW_NETWORK_DECLINE("L5");
				request1200 = IsoPackagerGlobal.request1200Enquiry(requestList);
			} else if ("REPEAT".equals(requestType)) {
				requestList = serviceImps.processSTP_IMPS_OW_NETWORK_DECLINE("L6");
			...
			for (Iterator<IsoV93Message> iterator = request1200.iterator(); iterator.hasNext();) {
				...
				dbresponse += serviceImps.updateSTP_IMPS_OW_NETWORK_DECLINE(isoV93MessageRes, requestType);
