...
@Service
public class STP_IMPS_TCC_DATA_IWscheduledTask {

	private static final Logger logger = LoggerFactory.getLogger(STP_IMPS_TCC_DATA_IWscheduledTask.class);
	@Autowired
...
//	@Scheduled(cron = "0 * * * * ?")
	public void executeTask() {
		logger.info("STP_IMPS_TCC_DATA_IWscheduledTask Scheduled task  start executed at: " + new Date());
	}

	public static void PROCESSSTP_IMPS_TCC_DATA_IW() {
		...
		try {
			...
			dbresponse += processRequestSTP_IMPS_TCC_DATA_IW("FRESH", "L4", serviceNetwork, dbresponse); // New requests
			System.out.println("NEW REQUEST END");

			dbresponse += processRequestSTP_IMPS_TCC_DATA_IW("ENQUIRY", "L5", serviceNetwork, dbresponse); // Enquiry
																											// requests
			System.out.println("ENQUIRED REQUEST END");

			dbresponse += processRequestSTP_IMPS_TCC_DATA_IW("REPEAT", "L6", serviceNetwork, dbresponse); // Repeat
																											// requests
	...
	private static int processRequestSTP_IMPS_TCC_DATA_IW(String requestType, String condition,
			ISOCommunicationExample serviceNetwork, int dbresponse) {
		List<STP_IMPS_TCC_DATA_IW> requestList = null;
