...
@Service
public class STP_IMPS_NTSL_NETSET_TTUMsheduledTask {

	private static final Logger logger = LoggerFactory.getLogger(STP_IMPS_NTSL_NETSET_TTUMsheduledTask.class);
	@Autowired
...
//	@Scheduled(cron = "0 * * * * ?")
	public void executeTask() {
		logger.info("STP_IMPS_NTSL_NETSET_TTUMsheduledTask Scheduled task  start executed at: " + new Date());
	}

	public static void PROCESSSTP_IMPS_NTSL_NETSET_TTUM() {
		...
		try {
			...
			dbresponse += processRequestSTP_IMPS_NTSL_NETSET_TTUM("FRESH", "L4", serviceNetwork, dbresponse); // New
																												// requests
			System.out.println("NEW REQUEST END");

			dbresponse += processRequestSTP_IMPS_NTSL_NETSET_TTUM("ENQUIRY", "L5", serviceNetwork, dbresponse); // Enquiry
			// requests
			System.out.println("ENQUIRED REQUEST END");

			dbresponse += processRequestSTP_IMPS_NTSL_NETSET_TTUM("REPEAT", "L6", serviceNetwork, dbresponse); // Repeat
			// requests
	...
	private static int processRequestSTP_IMPS_NTSL_NETSET_TTUM(String requestType, String condition,
			ISOCommunicationExample serviceNetwork, int dbresponse) {
		List<STP_IMPS_NTSL_NETSET_TTUM> requestList = null;
		...
		try {
			...
			for (Iterator<IsoV93Message> iterator = request1200.iterator(); iterator.hasNext();) {
				...
				dbresponse += serviceImps.updateSTP_IMPS_NTSL_NETSET_TTUM(isoV93MessageRes, requestType);
