...
public class IsoV93MessageRes implements IsoMessage {

	static Logger log = Logger.getLogger(IsoV93MessageRes.class.getName());
	...
	public IsoV93MessageRes() {
		// TODO Auto-generated constructor stub
	}

	public IsoV93MessageRes(MsgId msgId) throws Exception {
		this.msgId = msgId;
	}

	public IsoV93MessageRes(byte[] msgBytes) throws Exception {
//		 System.out.println(new String(msgBytes));
//		byte[] byteMsgId = ArrayUtils.subarray(msgBytes, 4, 8);
		byte[] byteMsgId = ArrayUtils.subarray(msgBytes, 12, 16);
		byte[] byteBitMap = ArrayUtils.subarray(msgBytes, 16, 32);
		byte[] byteData = ArrayUtils.subarray(msgBytes, 32, msgBytes.length);
		int currIndex = 0;
	...
	public byte[] generateMessage() throws Exception {
		...
		byte[] bytesHeader = "ISO015000010".getBytes();

//		System.out.println(baMsgId.length);
...
//	}
	...
	public void printMessage(String stan) throws Exception {
		Collections.sort(fields);
		System.out.println("****** FINACLE START RESPONSE " + stan + " *********");
		for (IsoField f : fields) {
...
//		System.out.println("****** FINACLE START REQUEST " + stan + " *********");
		System.out.println("****** FINACLE END RESPONSE " + stan + " *********");
	}
